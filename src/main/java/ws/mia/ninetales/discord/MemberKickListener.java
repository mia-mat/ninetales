...
public class MemberKickListener extends ListenerAdapter {
	...
	public MemberKickListener(MongoUserService mongoUserService, @Lazy DiscordLogService discordLogService, MojangAPI mojangAPI) {
	...
	public void onGuildMemberRemove(GuildMemberRemoveEvent event) {
		...
		if(ntUser.getDiscordApplicationChannelId() != null) {
			...
			discordLogService.info("Cleanup", "Closed discord application channel for <@%s> (`%s`) due to discord leave"
		...
		if(ntUser.getGuildApplicationChannelId() != null) {
			...
			discordLogService.info("Cleanup", "Closed guild application channel for <@%s> (`%s`) due to discord leave"
					.formatted(ntUser.getDiscordId(), mojangAPI.getUsername(ntUser.getMinecraftUuid())));
		}
		...
		if(ntUser.getTailDiscussionChannelId() != null) {
			...
			discordLogService.debug("Cleanup", "Closed tail discussion channel for <@%s> (`%s`) due to discord leave"
					.formatted(ntUser.getDiscordId(), mojangAPI.getUsername(ntUser.getMinecraftUuid())));
		}
		...
		if(ntUser.getQuestionChannelId() != null) {
			...
			discordLogService.info("Cleanup", "Closed question channel for <@%s> due to discord leave"
