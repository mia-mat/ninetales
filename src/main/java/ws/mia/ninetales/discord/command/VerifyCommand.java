...
public class VerifyCommand extends SlashCommand {
	...
	public VerifyCommand(MojangAPI mojangAPI, HypixelAPI hypixelAPI, MongoUserService mongoUserService) {
		super();
	...
	public CommandData getCommand() {
		return Commands.slash(COMMAND, "Let us know who you are :3").addOption(OptionType.STRING, "username", "Your Minecraft Username", true);
	}
	...
	public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
		if (event.getName().equals(COMMAND)) {
			...
			event.reply("Successfully verified you as `" + username.getAsString() + "`!\nWelcome to Ninetales :3").setEphemeral(true).queue();
